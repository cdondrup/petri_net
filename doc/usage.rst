...
The following describes a minimal example of how to use the Petri-Net framework. The benefits
of this framework will only really become apparent when the examples become a little
...
.. literalinclude:: ../rpn_recipe_planner_msgs/action/SimpleTest.action
  :language: YAML

So the action has an integer called ``value`` as a goal parameter and the result contains
an int called ``result``. There is no feedback. The message can be in ``rpn_recipe_planner_msgs/action/SimpleTest.action``.
...
in the ``rpn_recipe_planner/scripts/example_servers/simple_test.py`` and looks like this:

.. literalinclude:: ../rpn_recipe_planner/scripts/example_servers/simple_test.py
  :language: PYTHON
...
and the net will wait until it is finished before continuing. It receives input at start and produces output at the end. There is a second kind of server, 
however, in case your action server needs to communicate with an external knowledgebase at runtime.
...
The RPN server used here uses the same action as the other server above and looks like this:

.. literalinclude:: ../rpn_recipe_planner/scripts/example_servers/test.py
  :language: PYTHON
...
.. literalinclude:: ../rpn_recipe_planner/src/rpn_planner_kb/rpn_planner_knowledge_base.py
  :language: PYTHON
...
Petri-Nets are based on Plans and Domains. This is similar to PDDL style languages. An example domain can be found in ``rpn_recipe_planner/etc`` and looks like this:

.. literalinclude:: ../rpn_recipe_planner/etc/domains/domain.yaml
  :language: YAML
...
Now that we have defined all the actions we can use, we have to put them in some form of sequence. This plan could be produced by a proper planner but for now we will just hand craft it. The plan can be found in ``rpn_recipe_planner/etc`` and looks like this:

.. literalinclude:: ../rpn_recipe_planner/etc/plans/example.yaml
  :language: YAML
...
.. literalinclude:: ../rpn_recipe_planner_msgs/action/RosServer.action
  :language: YAML
...
Let's have a look at the example controller which can be found in the rpn_controller package:

.. literalinclude:: ../rpn_controller/scripts/example_controller.py
  :language: PYTHON
...
In oder to provide all the functionality the controller provides, we need to use a specific type of ROS action server that automatically registers itself with the controller and provides functionality to call the query and update methods using ROS services. This type of server can be found in ``rpn_controller_servers/abstract_controller_plugin_server.py`` and extends the ROS action server. The simple action server is not supported yet. 

.. literalinclude:: ../rpn_controller/src/rpn_controller_servers/abstract_controller_plugin_server.py
  :language: PYTHON
...
This server makes use of the methods in the ``utils.py`` file found in ``rpn_controller_servers/utils.py``.

The basic server
=====================

The simplest implementation of a server is the basic server ``rpn_controller_servers/basic_controller_plugin_server.py``:

.. literalinclude:: ../rpn_controller/src/rpn_controller_servers/basic_controller_plugin_server.py
  :language: PYTHON
...
In order to communicate with the controller, we will a use a different knowledge base implementation. As we have seen above, the external knowledge base can be customised to fit the need of the users. Initially, we went with a very simple implementation that just stores values given a name. Now we want to be able to not just do that but to also communicate with the controller if we choose to do so. For this purpose, we created the ``rpn_controller_knowledge_base.py`` wich can be found in ``rpn_controller_kb/rpn_controller_knowledge_base.py`` and looks like this:

.. literalinclude:: ../rpn_recipe_planner/src/rpn_controller_kb/rpn_controller_knowledge_base.py
  :language: PYTHON
...
We will updata the domain file used above to include a new knowledge base action that will commuictate with the controller. The new domain file called ``controller_domain.yaml`` can be found in ``rpn_recipe_planner/etc/domains/`` and looks like this:
...
The plan file ``rpn_recipe_planner/etc/plans/controller_example.yaml`` looks like this:

.. literalinclude:: ../rpn_recipe_planner/etc/plans/controller_example.yaml
  :language: YAML
...
The final thing we have to change is the ``rpn_recipe_planner/etc/worlds.yaml`` file to include our new domain and plan file:

.. literalinclude:: ../rpn_recipe_planner/etc/worlds.yaml
  :language: YAML
