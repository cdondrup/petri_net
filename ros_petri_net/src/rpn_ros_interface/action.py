...
class Action(object):
    ...
    def start(self):
        if self.server_thread is None or not self.server_thread.is_alive():
            self.server_thread = Thread(target=self.call_server)
    ...
    @property
    def succeeded(self):
        return self.get_result() == GoalStatus.SUCCEEDED

    @property
    def preempted(self):
        return self.get_result() in (GoalStatus.PREEMPTED, GoalStatus.PREEMPTING)

    @property
    def failed(self):
        return self.get_result() in (GoalStatus.LOST, GoalStatus.ABORTED)
